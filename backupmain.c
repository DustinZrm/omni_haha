...
uint8_t omniflag = 1;
uint8_t rx_data; // 用于存储接收到的数据
int omni = 0; //1为全向轮模式，0为自旋模式
double speed = 20;
...
void setMotorSpeed(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, TIM_HandleTypeDef* htim, uint32_t Channel, double motorSpeed) {
    ...
    if (motorSpeed > 0) {
        ...
        htim->Init.Period = PWM_PERIOD / motorSpeed; // 调节频率
        HAL_TIM_PWM_Init(htim); // 重新初始化PWM
        __HAL_TIM_SET_COMPARE(htim, Channel, htim->Init.Period / 2); // 重新设置占空比
    } else {
...
void handleCommand(uint8_t cmd) {
    switch (cmd) {
        ...
        case CMD_BACKWARD:
            ...
            break;
        case CMD_LEFT:
            if (omniflag)SpeedControl(-speed, 0, 0); //左移
            else SpeedControl(0, 0, -speed); // 左旋
            break;
        case CMD_RIGHT:
            if (omniflag)SpeedControl(speed, 0, 0); //右移
            else SpeedControl(0, 0, speed); // 右旋
            break;
        case CMD_Q:
            if (omniflag)SpeedControl(speed, speed, 0); // 左前方
            break;
        case CMD_E:
...
int main(void) {
    ...
    HAL_GPIO_WritePin(ST_GPIO_Port, ST_Pin, GPIO_PIN_SET);
    //设置PWM输出
    HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1);//A电机速度
    HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_1);//B电机速度
    HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_1);//C电机速度
    SpeedControl(0, 0, 0);
    //设置LED灯
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        handleCommand(rx_data); // 处理接收到的数据
